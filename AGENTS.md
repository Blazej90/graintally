...
Ten plik jest jedynym źródłem konwencji projektu. `CLAUDE.md` tylko go
importuje (`@AGENTS.md`), żeby Claude Code i Kimi Code czytały to samo —
zmiany wprowadzaj tutaj, nie tam.
...
- Transporty siedzą w `localStorage` (klucz `klosek-transports`). `firebase.ts`
  istnieje, ale **nic w `src/` go nie importuje** — Firestore czeka na Auth.
