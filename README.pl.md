...
![Vitest](https://img.shields.io/badge/testy%20jednostkowe-Vitest-6e9f18?logo=vitest&logoColor=white)
...
- **Jedna dostawa, jeden wynik** — transport jest ważony raz, a próbka pobierana
  z całego zestawu, więc kalkulator przyjmuje jeden tonaż i jeden zestaw
  parametrów z badania.
...
- **Przegląd transportów** — pogrupowane po zbożu, z wyszukiwaniem po nazwie
  (odpornym na literówki), filtrowaniem po dacie (dzień, zakres, miesiąc, rok)
  i skupującym oraz eksportem pojedynczego transportu do PDF.
...
src/
  ...
  lib/                       – zapis w localStorage, wyszukiwanie rozmyte, eksport PDF, cn()
  examples/example.ts        – przykładowe wyliczenia do wypisania w konsoli
tests/                       – testy Playwright (kalendarz, tryb edycji, kalkulator, PDF)
...
3. Walidacja wymagań progowych („spełnia / nie spełnia", np. kwas erukowy, GMO)
   ...
   `calculatePrice()`.
4. Tryb offline (PWA) i eksport historii do CSV (eksport pojedynczego
   transportu do PDF już działa).
